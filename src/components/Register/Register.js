...
export default function Register({
  ...
  return (
    <FormAuth
      linkHistory="/signin" title="Добро пожаловать!"
      buttonText={sendingData ? messageSendingData : 'Зарегистрироваться'}
      typeButton="signup" question="Уже зарегистрированы? &nbsp;"
      link="/signin" linkText="Войти"
      onSubmit={handleSubmit} isDisabled={!isValidForm}
      isMessageErrorAPI={isMessageErrorAPI}
    >

      <label
        className="auth-form__label" htmlFor="auth-input-name"
      >
        Имя
        <input
          ...
          disabled={isDisabledInput} required
        />
        <span
      ...
      <label
        className="auth-form__label" htmlFor="auth-input-password">
        Пароль
        <input
          className={`auth-form__input ${isDisabledInput ? 'auth-form__input_disabled' : ''}`}
